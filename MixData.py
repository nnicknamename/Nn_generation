...
class MixableDataset(Dataset):
    ...
    def __init__(self, datasets):
        ...
        self.idx_table = [
            [i for i, x in enumerate(self.targets == c) if x is True]
    ...
    def extract_targets(self):
        ...
        for dataset in self.datasets:
            targets.extend(
                map(lambda x: self.transform_label(x, dataset), dataset.targets)
            )
    ...
    def get_subDatast(self, focusClass, size=None):
        if size is None:
            ...
            assert size <= len(
                self.idx_table[focusClass]
            ), "the given size is larger than the focus class"

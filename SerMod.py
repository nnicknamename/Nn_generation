...
class SerMod(nn.Module):
    ...
    def parameter_vector(self, ser):
        ...
        for i in range(len(self.layer_spec) - 1):
            length = (
                self.layer_spec[i] * self.layer_spec[i + 1] + self.layer_spec[i + 1]
            )
            layer, ser = ser.split([length, len(ser) - length])
            l_weight, l_bias = layer.split(
                [self.layer_spec[i] * self.layer_spec[i + 1], self.layer_spec[i + 1]]
            )
            l_wMat = torch.reshape(
                l_weight, [self.layer_spec[i + 1], self.layer_spec[i]]
            )
            self.stack[k].load_state_dict(
                {"weight": l_wMat, "bias": l_bias}, strict=False
            )
            k += 2
    ...
    def serialize_layer(self, layer):
        ...
        # extract weights
        w = layer["weight"].clone().detach().flatten()
        # extract Biasis
        b = layer["bias"].clone().detach().flatten()
        res = torch.cat((w, b))
        return res
...
class Trainer:
    ...
    def create_model(self, data):
        model_vector, _ = data
        return light_serial_model(
            model_vector, self.model_spec, self.lr_rate, randomInint=self.random_init
        )

    def create_dataLoader(self, data):
        _, clas = data
        return DataLoader(
            dataset=self.train_dataset.get_subDatast(clas, self.train_dataset_size),
    ...
    def create_test_dataloader(self, data):
        _, clas = data
        return DataLoader(
            dataset=self.test_dataset.get_subDatast(clas),
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            pin_memory=True,
        )
    ...
    def train(self, data):
        ...
        model_trainer = pl.Trainer(
            callbacks=[EarlyStopping(monitor="train_loss", min_delta=0.0, mode="min")],
            max_epochs=self.nb_epochs,
    ...
    def get_test_results(self):
        assert (
            self.pretest_results is not None or self.posttest_results is not None
        ), "you did not run any tests"
    ...
    def train_models(self, data, pretest=False, posttest=False):
        ...
        if pretest:
            self.pretest_results = self.parallel_exec(
                self.test, zip(models, test_dataoaders)
            )

        self.parallel_exec(self.train, zip(models, train_dataloaders))
        if posttest:
            self.posttest_results = self.parallel_exec(
                self.test, zip(models, test_dataoaders)
            )
    ...
    def train(self, data, modelSpec):
        ...
        for i, d in enumerate(data):
            ...
            dataloader = DataLoader(
                self.train_dataset.get_subDatast(clas),
                num_workers=self.num_workers,
                batch_size=self.batch_size,
                shuffle=True,
                pin_memory=True,
